...
public class TokenApiTest {
    ...
    void givenCredentials_whenRequestTokenForUser_thenReturnJwtTokenWithOKStatus() {
        RestAssured.given()
                ...
                .baseUri("http://localhost").port(port).basePath("/securityoauth2server/api/v1/token/me/new")
                .when().get()
                .then()
    ...
    void givenToken_whenRequestToResource_thenReturnResourceWithOKStatus() {
        ...
        RestAssured.given()
                ...
                .baseUri("http://localhost").port(port).basePath("/securityoauth2server/api/v1/users")
                .when().get()
                .then()
                .statusCode(HttpStatus.OK.value())
                .body("size()", is(3))
    ...
    void givenClientId_whenRequestTokenForClient_thenReturnJwtTokenWithOKStatus() {
        ...
        RestAssured.given()
                ...
                .baseUri("http://localhost").port(port).basePath("/securityoauth2server/api/v1/clients")
                .body(givenBody)
                .when().post()
                .then()
        ...
        RestAssured.given()
                ...
                .basePath("/securityoauth2server/api/v1/token/me/{clientId}").pathParam("clientId", "test-client")
                .when().get()
                .then()
                .statusCode(HttpStatus.OK.value())
                .body("token", not(emptyOrNullString()))
