...
public class OauthScenarioTest {
    ...
    private String requestToGetToken() {
        ...
        var token = RestAssured.given()
                ...
                .baseUri(serverUrl).port(port).basePath("/securityoauth2server/api/v1/token/me/new")
                .when().get()
                .then()
    ...
    private String requestToGetTokenOfClient(String clientId) {
        ...
        var token = RestAssured.given()
                ...
                .basePath("/securityoauth2server/api/v1/token/me/{clientId}").pathParam("clientId", clientId)
                .when().get()
                .then()
                .statusCode(HttpStatus.OK.value())
                .body("token", not(emptyOrNullString()))
    ...
    private ClientDto registerClient() {
        ...
        RestAssured.given()
                .contentType(ContentType.JSON)
                .auth().basic(username, password)
                .baseUri(serverUrl).port(port).basePath("/securityoauth2server/api/v1/clients")
                .body(client)
                .when().post()
                .then()
    ...
    void getToken_getRequest() {
        ...
        RestAssured.given()
                ...
                .baseUri(serverUrl).port(port).basePath("/securityoauth2server/api/v1/health")
                .when().get()
                .then()
                .statusCode(HttpStatus.OK.value())
                .body(equalTo("UP"))
    ...
    void getGrantType_getToken_getRequest() {
        ...
        var response = RestAssured.given()
                .contentType(ContentType.JSON)
                .auth().basic(userName, password)
                .baseUri(serverUrl).port(port).basePath("/securityoauth2server/oauth2/authorize")
