...
@Aspect
@Component
public class MessageListenerWatcher {
    ...
    @Before("execution(* com.tutorial.springboot.listener.MessageListener.onMessage(..)) && args(event)")
    public void before(JoinPoint joinPoint, MessageEvent event) {
        events.put(keyGenerator.get(), new KeyValue<>(0, event));
    }

    @After("execution(* com.tutorial.springboot.listener.MessageListener.onMessage(..)) && args(event)")
